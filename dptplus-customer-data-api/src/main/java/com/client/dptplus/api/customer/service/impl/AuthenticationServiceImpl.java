package com.client.dptplus.api.customer.service.impl;

import com.client.dptplus.api.customer.component.HttpComponent;
import com.client.dptplus.api.customer.dto.KeycloakOAuthDto;
...
import com.client.dptplus.api.customer.exception.BadRequestException;
import com.client.dptplus.api.customer.exception.NotFoudException;
import com.client.dptplus.api.customer.model.UserRecoveryCode;
import com.client.dptplus.api.customer.repository.UserRecoveryCodeRepository;
import com.client.dptplus.api.customer.service.AuthenticationService;
...
public class AuthenticationServiceImpl implements AuthenticationService {
    ...
    public String auth(LoginDto dto) {
        try {
            ...
            ResponseEntity<String> response = httpComponent.restTemplate().postForEntity(
                    keycloakUri + "/realms/REALM_DPTPLUS_API/protocol/openid-connect/token",
                    request,
                    String.class
            );
    ...
    public void createAuthUser(UserRepresentationDto userRepresentation) {
        try {
            ...
            httpComponent.restTemplate().postForEntity(
                    keycloakUri + "/admin/realms/REALM_DPTPLUS_API/users",
                    request,
                    String.class
            );
        } catch (Exception e) {
    ...
    public void updateAuthUser(UserRepresentationDto userRepresentation, String currentEmail) {
        try {
            ...
            httpComponent.restTemplate().put(
                    keycloakUri + "/admin/realms/REALM_DPTPLUS_API/users/" + userId,
                    request
            );
    ...
    private String getUserAuthId(String currentEmail, HttpHeaders headers) throws JsonProcessingException {
        ...
        String responseGetUser = httpComponent.restTemplate().exchange(
                keycloakUri + "/admin/realms/REALM_DPTPLUS_API/users?email={email}&exact={exact}",
                HttpMethod.GET,
                request,
                String.class,
                uriVariables
